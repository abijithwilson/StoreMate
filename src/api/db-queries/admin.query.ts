...
export const fetchAdminProfile = `SELECT admin_id, first_name, last_name,
...
export const listAllStoreAdminQuery = (
  adminName,
  storeName,
  countryId,
  stateId,
...
AND mr.role_name='${
  K.STORE_ADMIN
}' AND ($1::text is null or UPPER(first_name) 
    LIKE UPPER('%${adminName}%')) AND 
    ($2::text is null or UPPER(ms.store_name) 
    LIKE UPPER('%${storeName}%'))      
    AND ($3::text is null or ms.country_id IN (${
  countryId ? JSON.parse(countryId).join(',') : null
}))  
    AND ($4::text is null or ms.state_id IN (${
  stateId ? JSON.parse(stateId).join(',') : null
})) 
    AND ($5::text is null or ms.district_id IN (${
  districtId ? JSON.parse(districtId).join(',') : null
})) 
group by a.admin_id 
...
export const StoreAdminProfileFetchQuery = `select  
m_admin.admin_id ,
m_admin.first_name ,
m_admin.last_name ,
m_admin.email ,
...
export const StoreAdminProfileUpdateQuery = `WITH updateTable AS (
   UPDATE m_admin
...
export const AssignedStoresQuery = (assignedStoreIdValues: string) =>
  `insert_stores AS (INSERT INTO
   t_admin_store_map (admin_id,store_id)
   values($1, unnest(array[ 
  ${assignedStoreIdValues} 
  ])))`;

export const DeleteStoresQuery = (
  unAssignedStoreIdValues: string
) => `delete_stores AS (
...
export const statementQuery = ` SELECT admin_id
    FROM t_admin_store_map
    WHERE admin_id=$1`;

export const deleteStoreAdminQuery = (idString: string) =>
  `
WITH 
...
`;
export const storeAdminInviteQuery = (
  assignedStoreIdValues: string
) => `WITH insertStoreAdmin AS(
...
export const singleOfferProductListQuery = (
  limit,
  offset,
  sortField,
  sortOrder,
  ...
  return {
    ...
    and ${
  assigned === 'true' ? '' : 'not '
}exists (select id from t_product_offer_map tpom
...
export const singleOfferProductAssignQuery = (assignedProductIdValues) => 
  `insert into
...
export const singleOfferProductUnassignQuery = `
