...
export const listAllStoresQuery = `SELECT m_store.store_id, m_store.store_name,
...
export const listSingleStoreQuery = `SELECT m_store.store_id,
 m_store.store_name,
...
export const nearbyStoresQuery = (
  userlatitude: number,
  userlongitude: number
) => ` SELECT * FROM (SELECT store_id,store_name,latitude,longitude,
...
export const rewardDetailsFetchQuery = `SELECT m_store.store_id,
...
export const insertStoreRewardPointsQuery = (assignedStoreIdValues: string) =>
  `INSERT INTO
   m_reward (per_visit_points,
...
export const fetchStoreWithoutRewardQuery = `SELECT m_store.store_id,
m_store.store_name
from m_store
...
export const StoreRewardUpdateQuery = (storeId: number[]) => {
  let queryValue = 3;
...
export const skuUnderStoreAndProductQuery = `select 
mst.id,
ms."name" as size,
mc."name" as colour ,
...
export const fetchStoreWiseProductListQuery = `select 
mp.id,
mp.product_name,
mp.category_id,
...
export const DeleteSkuUnderStore = (deletedStoreId) => {
  let queryValue = 0;
  return `
...
export const userProductFetchQuery = (
  ...
  if (filterCategory) {
    filterQuery.push(
      `AND category_id IN (${JSON.parse(filterCategory).map(() => {
        queryValue = queryValue + 1;
        return `$${queryValue}`;
      })})`
    );
...
export const storeOfferFetchDetailQuery = (
  sortField: string,
  sortOrder: string,
  limit: number,
  offset: number
...
export const unAssigninofferQuery = `
delete from t_offer_store_map
where store_id=$1 and offer_id=$2 
returning id`;

export const fetchStoresQuery = `select ms.store_id,
ms.store_name
from m_store ms
...
export const storeSectionPiechartQuery = (span) => `
with userDetails as (
  select
         tsv.entry_date_time,
         tsv.user_id,
...
export const vistorsLineGraphQuery = (span, limit, offset) => `
...
/*eslint-disable */
export const offerWiseProductFetchQuery = (
  filterCategory: string,
  filterColour: string,
  fiterSize: string
) => {
...
export const storeSectionBarchartQuery = (span, limit, offset) => `
with userDetail AS (
  select 
  tsv.user_id,
  tsv.entry_date_time ,
...
export const storeNameQuery = (storeName: string) => {
...
export const storeCountryFetchQuery = `
select 
ms.country_id,
c.country_name 
from m_store ms
...
export const storeStateFetchQuery = `
select 
ms.state_id,
s.state_name
from m_store ms
...
export const storeDistrictFetchQuery = `
select 
ms.district_id,
d.district_name
from m_store ms
...
export const fetchLastVisitedStoresQuery = (limit, offset) => 
  `with fetchStoreCount as (select 
    tsv.store_id,
    ms.store_name,
